...
public class BlockHighlightRenderer implements IRenderer {
    ...
    @Override
    public void onRenderWorldLast(Matrix4f modelViewMatrix, Matrix4f projectionMatrix) {
    ...
    // Minecraft 1.21.1 uses direct BufferUploader rendering, before RenderPipeline.
    private void drawWithMaLiLib(Vec3 cameraPos, List<HighlightEntry> entries,
        ...
        try {
            if (hasFilled) {
                ...
                for (HighlightEntry e : entries) {
                    if (e.style != HighlightStyleType.OUTLINE) {
                        addFilledBoxModern(buffer, e.pos, e.r, e.g, e.b,
                                (int) (e.alpha * 255), cameraPos);
                    }
                }
            ...
            if (hasOutline) {
                ...
                for (HighlightEntry e : entries) {
                    if (e.style != HighlightStyleType.FILLED) {
                        addOutlineBoxLines(buffer, e.pos, e.r, e.g, e.b,
                                (int) (e.alpha * 255), cameraPos);
                    }
                }
                MeshData mesh = buffer.build();
