...
public final class ServuxHandItemConfirmation {
    ...
    public static synchronized HandConfirmationGate.Status verify(
        ...
        if (desiredHotbarSlot < 0) {
            desiredHotbarSlot = player.getInventory().selected;
        }
        expectedItemIds = acceptedIds;

        EntitiesDataStorage dataManager = EntitiesDataStorage.getInstance();
    ...
    public static synchronized void handleEntityData(int entityId, CompoundTag data) {
        if (!isEnabled()
                || data == null
        ...
        int serverSelectedSlot = (data.contains("SelectedItemSlot", 99) ? data.getInt("SelectedItemSlot") : -1);
    ...
    private static String inventoryItemAt(CompoundTag data, int slot) {
        if (slot < 0 || !data.contains("Inventory", 9)) {
            return AIR_ID;
        }
        ListTag inventory = data.getList("Inventory", 10);
        for (int index = 0; index < inventory.size(); index++) {
            CompoundTag entry = inventory.getCompound(index);
            if ((entry.contains("Slot", 99) ? entry.getInt("Slot") : -1) == slot) {
                return (entry.contains("id", 8) ? entry.getString("id") : AIR_ID);
            }
        }
        return AIR_ID;
    }
