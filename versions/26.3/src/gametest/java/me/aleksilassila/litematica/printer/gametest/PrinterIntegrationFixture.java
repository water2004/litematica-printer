...
/** Shared world setup and assertions for printer integration tests. */
@SuppressWarnings("UnstableApiUsage")
final class PrinterIntegrationFixture {
    static final BlockPos PLACE_TARGET = new BlockPos(2, 64, 0);
    static final BlockPos BREAK_TARGET = new BlockPos(3, 64, 0);
    static final int MATERIAL_COUNT = 4;
    static final int MAIN_INVENTORY_SHULKER_SLOT = 9;

    private PrinterIntegrationFixture() {}

    static void prepareWorld(TestSingleplayerContext singleplayer,
    ...
    static boolean inventoryArrived(ItemStack materialStack, boolean useQuickShulker) {
        if (useQuickShulker) {
    ...
    static void configureQuickShulker() {
    ...
    static void configureFillPrinter() {
    ...
    static void disablePrinter() {
        Configs.Core.WORK_SWITCH.setBooleanValue(false);
        Configs.Fill.ENABLED.setBooleanValue(false);
        ModuleManager.FILL.resetScanState();
    }

    static int countStoredStone(ItemStack stack) {
    ...
    static PlacementResult readPlacementResult(TestSingleplayerContext singleplayer,
                                               int shulkerSlot) {
    ...
    static boolean isInventoryFull(net.minecraft.world.entity.player.Inventory inventory) {
    ...
    static int countLooseStone(net.minecraft.world.entity.player.Inventory inventory) {
        int count = 0;
        for (int slot = 0; slot < Math.min(36, inventory.getContainerSize()); slot++) {
    ...
    static void assertQuickStorageCapability(String expectedQuickShulker) {
    ...
    static boolean usesDirectProtocol(String quickShulkerMode) {
        return quickShulkerMode.equals("current")
                || quickShulkerMode.equals("direct");
    }

    record PlacementResult(boolean placed, int directStone, int boxedStone,
